...
public class StudentDaoImpli implements StudentDao {
	...
	@Override
	public void passChange(final String pass, final int id) {
		hibernateTemplate.execute(new HibernateCallback<List<Login>>() {
			public List<Login> doInHibernate(Session session) throws HibernateException {
				Query q = session.createQuery("update Login set password=? where id=?");
				q.setString(0, pass);
				q.setInteger(1, id);
				q.executeUpdate();
				session.flush();
