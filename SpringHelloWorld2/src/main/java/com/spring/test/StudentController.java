...
public class StudentController {
	...
	@RequestMapping(value = "re_exam")
	public String reexam() {
		return "student/reexam";
	}
	...
	@RequestMapping(value = "/le_ave")
	public String getleaveform(HttpSession session) {
		Login l = (Login) session.getAttribute("login");
		List<Leaver> listl = studentService.showleave(l.getId());
	...
	public String giveleave(Leaver leave, HttpSession session) {
		if (leave.getReason() != null) {
			...
			List<Leaver> listl = studentService.showleave(leave.getUid());
			session.setAttribute("leave", listl);
		}
		return "student/leave";
	}

	@RequestMapping(value = "pass_chan")
	public String passchange(Login logi) {
		System.out.println(logi.getFirstname());
		System.out.println(logi.getId());
		//int id=Integer.parseInt(id1);
		studentService.changepass(logi.getFirstname(),logi.getId());
		return "home/login";
	}
